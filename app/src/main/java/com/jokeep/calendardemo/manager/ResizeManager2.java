 ...
 *
 */
public class ResizeManager2 {

	public static final String TAG = "ResizeManager";

	/** View to resize */
	@NonNull private MyCollaspCalendarView mCalendarView;
	...
	@Nullable private ProgressManager2 mProgressManager;

	public ResizeManager2(@NonNull MyCollaspCalendarView calendarView) {
		...
		if (Math.abs(calculateXDistanse(ev)) > Math.abs(calculateDistance(ev))) {
			...
			if (Math.abs(yDIff) > mTouchSlop) { // FIXME this should happen only if dragging int right direction
				...
				if (mProgressManager == null) {
					...
					mProgressManager = new ProgressManagerImpl2(mCalendarView, weekOfMonth, state == CalendarManager.State.MONTH);
				}
				return true;
			}
		}
